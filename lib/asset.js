...
const
	DEFAULT_HOST = 'master-assetapi.scaffold-workers-ext-main-us.m.cld.octavelive.com',
	DEFAULT_SECURE = true,
	EVENT_ERROR = 'error',
...
module.exports = function (assetOptions, ensureAuthHeaders, self) {
	...
	settings.secure =
		validation.isEmpty(assetOptions) || typeof assetOptions.secure !== 'boolean' ?
			DEFAULT_SECURE :
			assetOptions.secure;
	...
	function buildAssetAlias (asset) {
		let
			isrc = validation.coalesce(asset.isrc, asset.ISRC),
			parts = [],
			upc = validation.coalesce(asset.upc, asset.UPC);
		...
		if (!validation.isEmpty(isrc)) {
			parts.push('isrc', isrc);
		}

		if (!validation.isEmpty(upc)) {
			parts.push('upc', upc);
		}
	...
	self.checkAsset = (asset, options, callback) => {
	...
	self.getOriginalByAsset = (asset, options, callback) => {
		// handle any non-specified input params
		if (typeof options === 'function') {
			callback = options;
			options = undefined;
		...
		let exec = co(function *() {
			...
			return yield req.get({
				headers : headers,
				pathname : `/v1/assets/${buildAssetAlias(asset)}/original`,
				query : options
			});
		});
	...
	self.upsertOriginals = (originals, callback) => {
		// handle any non-specified input params
		...
		let exec = co(function *() {
			if (validation.isEmpty(originals)) {
				return yield Promise.reject(new Error('originals are required'));
			}
