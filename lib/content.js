...
module.exports = function (contentOptions, ensureAuthHeaders, self) {
	...
	self.getAssetStream = (track, options, callback) => {
		...
		let exec = co(function *() {
			...
			contentUrl = yield req.get({
				headers : headers,
				pathname : `/v0/assets/${key}`,
				query : options
			});

			if (validation.isEmpty(contentUrl)) {
				return reject(new Error('unable to determine content URL'));
			}

			// capture stream of content and return that
			return https.get(contentUrl, (contentStream) => {
				return Promise.resolve(contentStream);
			}).on('error', (err) => {
