var
	...
	CollectionProxy = require('./collection'),
...
module.exports = (function (self) {
	...
	self.configure = (clientId, secret, options) => {
		...
		self.collection = new CollectionProxy(options.collection, key.ensureAuthHeaders);
		...
		self.track = new TrackProxy(options.track, key.ensureAuthHeaders);

		// create a mechanism to retrieve configured options
		self.options = () => ({
			asset : self.asset.settings(),
			claim : self.claim.settings(),
			collection : self.collection.settings(),
