...
const
	DEFAULT_HOST = hosts.LOCATION_API,
	DEFAULT_SECURE = true,
...
module.exports = function (locationOptions, ensureAuthHeaders, self) {
	...
	self.deletePhysicalLocation = (physicalLocationId, callback) => {
		// handle any non-specified input params
	...
	self.getLocationsByAliases = (locationAliasList, callback) => {
		// handle any non-specified input params
		if (typeof locationAliasList === 'function') {
			callback = locationAliasList;
			locationAliasList = undefined;
		}

		let exec = co(function *() {
			if (validation.isEmpty(locationAliasList)) {
				return yield Promise.reject(new Error('locationAliasList is required'));
			}
