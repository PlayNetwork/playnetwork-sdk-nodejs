...
module.exports = function (musicOptions, ensureAuthHeaders, self) {
	...
	self.addPlaylistTracks = (playlistId, tracks, callback) => {
		...
		let exec = co(function *() {
			...
			if (validation.isEmpty(tracks)) {
				return Promise.reject(new Error('tracks are required'));
			}
	...
	self.allCollectionTracks = (collectionId, query, callback) => {
		...
		let exec = co(function *() {
			if (validation.isEmpty(collectionId)) {
				return Promise.reject(new Error('collectionId is required'));
			}
	...
	self.createPlaylist = (playlist, callback) => {
		...
		let exec = co(function *() {
			if (validation.isEmpty(playlist)) {
				return Promise.reject(new Error('playlist is required'));
			}

			if (validation.isEmpty(playlist.title)) {
				return Promise.reject(new Error('playlist title is required'));
			}
	...
	self.deleteBroadcast = (stationId, broadcastId, callback) => {
		...
		let exec = co(function *() {
			...
			if (validation.isEmpty(broadcastId)) {
				return Promise.reject(new Error('broadcastId is required'));
			}
	...
	self.deletePlaylistTrack = (playlistId, track, callback) => {
		...
		let exec = co(function *() {
			...
			if (validation.isEmpty(alias)) {
				return Promise.reject(new Error('track is missing identifier'));
			}
	...
	self.getTrack = (trackAlias, query, callback) => {
		// handle any non-specified input params
		if (typeof query === 'function') {
			callback = query;
			query = undefined;
		...
		let exec = co(function *() {
			if (validation.isEmpty(trackAlias)) {
				return Promise.reject(new Error('trackAlias is required'));
			}
	...
	self.getTracks = (tracks, callback) => {
		// handle any non-specified input params
		if (typeof tracks === 'function') {
			callback = tracks;
			tracks = undefined;
