	...
	self.recordPlay = (playback, callback) => {
		...
		let exec = co(function *() {
			if (validation.isEmpty(playback)) {
				return Promise.reject(new Error('playback data is required'));
			}

			if (validation.isEmpty(playback.content)) {
				return Promise.reject(new Error('content information is required'));
			}
			...
			if (validation.isEmpty(playback.content.assetId) &&
				...
				return Promise.reject(new Error('content identifier is required'));
			}
			...
			if (validation.isEmpty(playback.deviceId) &&
				...
				return Promise.reject(new Error('device identifier is required'));
			}
